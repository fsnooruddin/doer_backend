...
def query_api(term, location, searchlimit):
    ...
    tfile = open(output_prefix + "json.curl", "w")
...
def getOpeningSchedule():
    ...
    for x in range(2):
        ...
        out = ""
        out = day + ": " + str(open_time) + "-" + str(close_time)
...
def write_html_business(fh, tfh, response):
    ...
    tfh.write("curl --header \"Content-Type: application/json\" \\"); 
    tfh.write("--request POST \\");
    ...
    tfh.write('"services": "{0}"'.format(ocats))
    tfh.write(",")
    ...
    tfh.write("http://localhost:8080/api/doers\n");
