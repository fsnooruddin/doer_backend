...
async function addCostToJob(req, res) {
	...
	if (Utils.validateIntegerParam("Job Id", id) == false) {
		logger.error("invoice-controller addTipToJob missing job Id or job Id not integer: " + id);
		res.status(400).send({ message: "Error addTipToJob - Job Id is missing or job id is not integer" });
		return;
	}
	logger.info("Invoice-controller addCostToJob id = " + id + " new cost is: " + req.body);
...
async function addTipToJob(req, res) {
	...
	logger.info("Invoice-controller addTipToJob id = " + id + " new tip is: " + req.body);
	try {
		const new_cost = await JobCost.create(req.body);
		logger.info("Invoice-controller addTipToJob id = " + id + "  returning " + JSON.stringify(new_cost));
		res.status(200).send(new_cost);
	} catch (err) {
		res.status(500).send({
			message: "Error retrieving addTipToJob for job id=" + id + " error: " + err.message,
		});
	}
}
